...
この場合、ipテーブルにはIPv6固有のルールは書けませんし、ip6テーブルにIPv4固有のルールは書けません。
...
通常IPv6のNext HeaderにはTCPなら6、UDPなら17、ICMPなら58が入りますが、拡張ヘッダ(可変長)がある場合はそのヘッダを示すプロトコル番号となり、拡張ヘッダのNextHeaderに本来入る6や17が指定されます。

Next Headerを保持するかしないかはプロコル番号に依存し、TCPヘッダが次に指定された場合はTCPヘッダの仕様の通りNext Headerは持たずにヘッダのあとにはデータグラムがくることになります。

カプセル化される時には、続くデータグラムからカプセル化を解いた後のヘッダのプロトコル番号が入っていることもあります。

IPv6の拡張ヘッダには、ルーティングを示すRouting Header(43)、送信者が生成した(IPv6では経路中に新たにフラグメントはおきません)フラグメントを示すFragment Header(44)などがあります。

これらはプロトコルではないですが、IANA の「Protocol Numbers」には登録されています。

前置きが長くなりましたが、まずこのような経緯で名前が違うので指定の仕方が変わります。IPv4の場合は、フィールド名がプロトコル番号なので前述の通り次のような指定方法になります。
```
ip  protocol icmp accept
```
...
```
となり、この書式は有効です。しかしこの書き方だと問題が起きることがあります。それは、先ほどあったようにNext Headerは複数存在することがあるというところから来ます。

ip6 nexthdrは先頭のnext headerしか判定しません。そのため拡張ヘッダがあると取りこぼしてしまいます。そのため前述のような記述となります。
```
meta l4proto ipv6-icmp accept
```
これは最終的なNext Headerの値を判定するための書式です。

ちなみに、拡張ヘッダのどこかに存在するルーティングヘッダをルール化するなら
```
exthdr rt exists accept
```
とします。なんとなくこれにipv6-icmp を指定してもいいような気がしますが、ipv6-icmpは *拡張* ヘッダではなく普通のヘッダなので構文エラーとなります。
...
ct(Connection Tracking:接続追跡)の略で、様々な状態判定ができます。stateの他には、同時接続数を制限する
...
ちなみにiptablesやnftablesのESTABLISHEDはTCPにおけるESTABLISHEDとは意味合いが異なり、UDPにおいてもESTABLISHEDの概念があります。
...
### トンネル時の挙動
これは iptables時代から変わりはありませんが、トンネル処理をしているサーバーの場合は、カプセル化された外側のデータはinputフィルターに入り、カプセル化を解除したデータは解除後の宛先に応じて、inputまたはforwardにはいります。
