...
通常IPv6のNext HeaderにはTCPなら6、UDPなら17、ICMPなら58が入りますが、拡張ヘッダ(可変長)がある場合はそのヘッダを示すプロトコル番号となり、拡張ヘッダのNext Headerに本来6や17が指定されます。

基本的に拡張ヘッダにあたるデータは、さらにNext Headerを保持し、次に伝播するようになっています。

TCPヘッダの場合は、従来のTCPヘッダの仕様の通りNext Headerはフィールドはありません。ヘッダのあとにはデータグラムがくることになります。
...
ip6 nexthdrは先頭のNext Headerしか判定しません(IPヘッダの次のヘッダしか判定しません)。そのため拡張ヘッダが複数あると取りこぼしてしまいます。そのため次のような記述となります。
```
meta l4proto ipv6-icmp accept
```
