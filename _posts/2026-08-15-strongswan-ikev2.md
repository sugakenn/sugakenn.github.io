...
## ローカルインターフェースと同じネットワークに所属させたい

IPv4環境では、strongSwanのローカルインターフェース側のアドレスと同じネットワーク帯に所属させたい場合があるかもしれません。

その際は`farpプラグイン`を使います。これは、`libcharon-extra-plugins`に含まれるもので、ここまでではインストールしていませんので新たに加えます。

これによりプラグインがインストールされ`/etc/strongswan.d/charon`に、`farp.conf`と`dhcp.conf`などの新たな設定ファイルが入ります。

この状態で、先ほどの設定のアドレスプールを、strongSwanの内側のアドレス帯を指定することで、リモート端末がstrongSwanと同じネットワークにいるような扱いになります。

新たに加わった設定ファイルにはデフォルトでloadされるように記述されているので、あとはstrongSwanを再起動するだけで使えます。

`dhcp.conf`の方は、この時のアドレスの配布をネットワーク上のDHCPサーバーから受けたいという場合に利用するプラグインです。

プラグインが読み込まれている環境で、アドレスプールの指定に`dhcp`とすると、DHCP経由でアドレスを取得するようになります。


farpは`Fake Aarp`の略だそうです。ARP代理応答(Proxy ARP)と似たもので、strongSwanがリモートの端末の変わりにARPに返信し、端末宛のデータを一旦受け取って転送する仕組みです。

ブリッジ接続とは違うので(ネットワーク同士の接続でもないですが)、マジックパケットなどをリモート側から発信してもstrongSwan側には流れません。