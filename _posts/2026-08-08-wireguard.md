...
```
上記で、システムログのサービスコントロールマネージャーが生成するイベントID 7045(サービスのインストール)の中から、サービス名が`WireGuard Tunnel: wg0`、という指定になります。
...
その際は、`and Data[@Name='StartType']='xxx'`も条件に入れる必要があると思いますが、イベントビューアのWindowsログ、システム、詳細EventDataを確認して調整します。

SYSTEM権限で、ユーザーがログオンしているかどうかにかかわらず実行、最上位の特権で実行するを付けます。

また、サービスが起動状態で再起動をかけた時は、イベントログは出ないので補足できません。
それ用に[スタートアップ時]に[遅延]をつけて、同じスクリプトを起動させるようにしておきます。
