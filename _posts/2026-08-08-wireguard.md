...
そこで私が考えたのは、通常は子側のKEEP ALIVEを3600(1時間)にしておき、必要時は中央側から25秒毎のKEEP ALIVEを送信する方法です。

回線を使いたくなったら中央側で対象のインターフェスに対してPersistentKeepaliveを25に設定します。
...
## バックアップ回線で使う場合の方法2
先の方法では、中央サーバーへのアクセスにDDNSを使う環境の場合、動かなくなるケースがあることがわかりました。

WireGurdは初回起動時だけにDNS解決をするので、起動後に中央サーバーのIPアドレスが変わってしまうと接続が途絶えます。

ポートフォワードを設定していれば中央サーバーからの発信で接続が回復できますが、そうしようとした時に中央サーバーに記憶している子側のアドレスが変わっていれば接続できません。

つまり2拠点同時にIPアドレスが変更されると通信ができなくなるということです。

KeepAliveはもともと、接続を維持する目的のものなので限界があります。そこで、WireGurad Beaconサーバー(死活監視サーバー)を中央サーバーに設定する方法を考えました。

DDNSのように子側のIPを記憶しておくものですが、そのままではNAT越えができないので、各拠点ではルーターにポートフォワードを設定して対象の端末のWGへ到達させる設定を入れる必要があります。

そうしておいて、接続をしたい場合は中央サーバーからコネクションを貼ります。
